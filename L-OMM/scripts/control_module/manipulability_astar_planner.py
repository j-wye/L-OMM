#!/usr/bin/env python3
"""Manipulability-cost A* planner policy over a reduced active-manifold map."""
...
class ManipulabilityAStarPlanner:
    """Stateless manipulability-regularized active-map A* planner policy."""
    ...
    def plan(self, request: PlanRequest) -> PlanResult:
        ...
        cost_mode = str(request.cost_mode).strip().lower()
        if cost_mode not in ("distance", "manipulability"):
            raise ValueError(f"unknown cost_mode: {request.cost_mode}")
        mu_safe = self._resolve_mu_safe(handle, snapshot.base_feasible_mask, request)
        cost_config = {
            "planner_cost_mode": cost_mode,
            "manip_weight": float(request.manip_weight),
            "manip_kappa": float(request.manip_kappa),
            "manip_mu_safe": float(mu_safe),
            "manip_mu_safe_percentile": float(request.manip_mu_safe_percentile),
            "manip_penalty_form": "normalized_harmonic_tanh",
        }

        if projection_ok:
            cells, cost, expanded, time_ms = self._astar(
                feasible, start_cell, goal_cell, handle, request, float(mu_safe)
            )
            if not cells:
                valid_grasp = False
        ...
        feasibility_stats.update(cost_config)
        return PlanResult(
            found=bool(cells),
            cells=list(cells),
            xz=xz,
    ...
    def _resolve_mu_safe(self,
                         handle: MapHandle,
                         base_feasible: np.ndarray,
                         request: PlanRequest) -> float:
        explicit = float(request.manip_mu_safe)
        if explicit > 0.0 and math.isfinite(explicit):
            return explicit
        values = handle.mu_grid[np.asarray(base_feasible, dtype=bool)]
        values = values[np.isfinite(values)]
        values = values[values > float(request.map_snapshot.mu_min)]
        if values.size == 0:
            return float(request.map_snapshot.mu_min)
        pct = float(np.clip(request.manip_mu_safe_percentile, 0.0, 100.0))
    ...
    def _edge_cost(self,
                   ...
                   request: PlanRequest,
                   mu_safe: float) -> float:
        if str(request.cost_mode).strip().lower() != "manipulability" or float(request.manip_weight) <= 0.0:
            return float(base_cost)
        mu_i = float(handle.mu_grid[int(cur[0]), int(cur[1])])
        mu_j = float(handle.mu_grid[int(nxt[0]), int(nxt[1])])
        denom = mu_i + mu_j
        if not (math.isfinite(mu_i) and math.isfinite(mu_j)) or denom <= 0.0:
            return float("inf")
        mu_e = (2.0 * mu_i * mu_j) / denom
        denom_safe = float(mu_safe) - float(request.map_snapshot.mu_min)
        if denom_safe <= 0.0:
            return float(base_cost)
        deficit = max(0.0, (float(mu_safe) - mu_e) / denom_safe)
        phi = math.tanh(float(request.manip_kappa) * deficit)
        return float(base_cost * (1.0 + float(request.manip_weight) * phi))
    ...
    def _astar(self,
               ...
               handle: MapHandle,
               request: PlanRequest,
               mu_safe: float) -> Tuple[List[Cell], float, int, float]:
        ...
        while heap:
            ...
            for dx, dz, scale in self.NEIGHBORS:
                ...
                if closed[nxt] or not feasible[nxt]:
                    continue
                step_cost = self._edge_cost(cur, nxt, scale * res, handle, request, mu_safe)
